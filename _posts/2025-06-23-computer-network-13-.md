...
출퇴근 시간 도로가 막히는 것과 네트워크 혼잡은 원리가 똑같다 — 한 지점에 처리할 수 있는 양보다 많은 트래픽이 몰리면 지연이 커지고, 임계점을 넘으면 오히려 전체 처리량(Throughput)이 무너진다. 이 글은 혼잡이 왜 발생하고, 왜 방치하면 처리량이 "0"까지 떨어지는지, 그리고 네트워크가 이를 어떻게 감지하고 제어하는지를 정리한다.
...
## 핵심 정리
- 혼잡 제어의 모든 기법은 결국 "누가 알아채고(암시적/명시적), 어느 방향으로 알리고(역방향/순방향), 어떻게 대응하는가(속도 감소/패킷 폐기)"라는 세 가지 축의 조합이다.
- Backpressure는 홉 단위로 전파되는 강력한 방식이지만 연결 지향 네트워크에서만 쓸 수 있고, IP 기반 인터넷처럼 비연결형 네트워크에서는 지연 증가·패킷 손실을 관찰하는 암시적 방식(TCP의 혼잡 제어가 대표적)에 의존할 수밖에 없다.
- Moderate Congestion과 Severe Congestion을 가르는 결정적 차이는 "재전송"이다 — 버퍼가 넘쳐 패킷이 버려지기 시작하면 재전송 트래픽이 혼잡을 더 심화시키는 악순환이 시작되고, 이 지점을 넘으면 처리량이 오히려 감소한다.
