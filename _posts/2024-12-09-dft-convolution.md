...
[푸리에 변환의 핵심 수식](/discrete-fourier/)에서 정리한 F(u,v)=ΣΣf(x,y)·e^(-i2π(ux/M+vy/N)) 수식을 실제 이미지에 적용하면 어떤 일이 벌어지는지를 이 글에서 다룬다. **DFT 기반 Convolution**은 이미지에서 특정 패턴을 **강조하거나 억제**하는 데 사용된다. 결과적으로 **필터에 따라 이미지에서 원하는 패턴을 추출**할 수 있다.
...
## 왜 공간 영역에서 바로 컨볼루션하지 않고 주파수 영역을 거칠까

공간 영역에서 컨볼루션은 커널을 이미지 전체에 슬라이딩시키며 픽셀마다 곱하고 더하는 연산이라 이미지 크기와 커널 크기에 비례해 계산량이 커진다. 반면 주파수 영역에서는 "이미지의 DFT × 필터의 DFT"라는 단순 원소별 곱셈 한 번으로 같은 결과를 얻을 수 있다(컨볼루션 정리). 커널이 크거나 이미지 해상도가 높을수록 이 차이가 실질적인 속도 차이로 이어지기 때문에, 큰 블러·샤프닝 필터는 실무에서 주파수 영역 처리로 구현되는 경우가 많다.

## 핵심 정리
- DFT 기반 컨볼루션의 핵심은 "공간 영역의 합성곱 = 주파수 영역의 곱셈"이라는 컨볼루션 정리다.
- 절차는 이미지 DFT → 필터 제로패딩 후 DFT → 두 결과를 곱함 → IFFT로 공간 영역 복원, 네 단계로 고정되어 있다.
- 필터의 통과 대역(저주파/고주파/특정 대역)을 바꾸는 것만으로 블러링, 경계 강조, 노이즈 억제 등 서로 다른 효과를 낼 수 있다.
