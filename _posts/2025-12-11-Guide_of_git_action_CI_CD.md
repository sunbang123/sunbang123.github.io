...
categories: 
tags:  ["Git", "DevLog"]
description: "GitHub Pages 블로그에 GitHub Actions를 연결하며 겪은 SSH Deploy Key, Secret 설정, 자동 커밋 권한 문제 해결 기록입니다."
experience_note: "GitHub Pages 블로그에 최근 커밋 목록을 자동으로 반영하려고 직접 GitHub Actions 워크플로우를 구성하며 겪은 시행착오를 정리했습니다."
---

# GitHub Actions CI/CD 자동화

이 글은 GitHub Pages 블로그에 GitHub Actions를 붙이면서 실제로 겪은 문제를 정리한 기록입니다. 목표는 최근 커밋 활동을 자동으로 가져와 블로그에 반영하는 것이었고, 구현 중 SSH Deploy Key 설정, Secret 이름 불일치, 자동 커밋 권한 문제를 차례로 만났습니다.

단순히 CI/CD 개념을 설명하기보다, 어떤 설정에서 막혔고 어떤 기준으로 해결했는지를 남겨 같은 구조를 다시 만들 때 확인할 수 있게 했습니다.
